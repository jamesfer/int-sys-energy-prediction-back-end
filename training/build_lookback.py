...
def build_lookback_set(data_set, start, end, lookback):
...
def filter_range(data_set, end, start):
    included = {k: num_or_none(v) for k, v in data_set.items()
                if start <= k <= end}
    excluded = {k: num_or_none(v) for k, v in data_set.items()
                if k < start or k > end}
...
def convert_to_num(data_set):
    return {k: num_or_none(v) for k, v in data_set.items()}
